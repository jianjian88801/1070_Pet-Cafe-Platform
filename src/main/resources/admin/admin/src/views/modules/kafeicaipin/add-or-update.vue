...
export default {
  ...
  methods: {
	...
	this.ruleForm.jieshao = this.ruleForm.jieshao.replace(reg,'../../../springbootx9rzc/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `kafeicaipin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kafeicaipinCrossAddOrUpdateFlag = false;
                  this.parent.search();
