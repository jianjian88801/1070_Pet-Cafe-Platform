<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="互动项目" prop="hudongxiangmu">
          <el-select v-model="ruleForm.hudongxiangmu" placeholder="请选择互动项目">
            <el-option
                v-for="(item,index) in hudongxiangmuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="互动项目" prop="hudongxiangmu">
	      <el-input v-model="ruleForm.hudongxiangmu"
                placeholder="互动项目" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="体验时间" prop="tiyanshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.tiyanshijian" 
                type="datetime"
                placeholder="体验时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tiyanshijian" label="体验时间" prop="tiyanshijian">
              <el-input v-model="ruleForm.tiyanshijian" 
                placeholder="体验时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tupian = true;
          ...
          if(o=='hudongxiangmu'){
            this.ruleForm.hudongxiangmu = obj[o];
	    this.ro.hudongxiangmu = true;
            continue;
          }
          if(o=='tiyanshijian'){
            this.ruleForm.tiyanshijian = obj[o];
	    this.ro.tiyanshijian = true;
            continue;
          }
          if(o=='beizhu'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/hudongxiangmu/xiangmu`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.hudongxiangmuOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `chongwutiyan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.chongwutiyanCrossAddOrUpdateFlag = false;
                  this.parent.search();
